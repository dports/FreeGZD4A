...
public class ModSelectDialog {
    ...
    dialog.setOnKeyListener(
        (arg0, keyCode, event) -> {
          ...
          if (keyCode == KeyEvent.KEYCODE_BACK) {
            if (extraPath.isEmpty() || !extraPath.contains("/")) return false;
            else {
    ...
    listView.setOnItemClickListener(
        (parent, view, position, id) -> {
          ...
          } else // select/deselect
          {
            ...
            if (!removed) selectedArray.add(extraPath + "/" + filesArray.get(position));
    ...
    listView.setOnItemLongClickListener(
        (parent, view, position, id) -> {
          if (filesArray.get(position).startsWith("/")) {
            ...
            if (!removed) selectedArray.add(extraPath + "/" + name);
  ...
  public String getResult() {
    ...
    if (PrBoomMode && selectedArray.size() > 0) result = new StringBuilder("-file");

    for (int n = 0; n < selectedArray.size(); n++) {
      if (PrBoomMode) result.append(" ").append(selectedArray.get(n));
      else {
        if ((selectedArray.get(n).endsWith(".deh")) || (selectedArray.get(n).endsWith(".bex")))
          result.append("-deh ").append(selectedArray.get(n)).append(" ");
        else result.append("-file ").append(selectedArray.get(n)).append(" ");
      }
    }
