...
public class ControlInterpreter {
  ...
  HashMap<Integer, Boolean> analogButtonState =
      new HashMap<>(); // Saves current state of analog buttons so all sent each time
  ...
  public ControlInterpreter(
      ...
      ArrayList<ActionInput> gamepadActions,
      String controlfile,
      boolean ctrlEn) {
    ...
    for (ActionInput ai : config.actions) {
      if ((ai.sourceType == Type.ANALOG)
          && ((ai.actionType == Type.MENU) || (ai.actionType == Type.BUTTON))) {
  ...
  public boolean onKeyDown(int keyCode, KeyEvent event) {
    ...
    if (gamePadEnabled) {
      for (ActionInput ai : config.actions) {
        if (((ai.sourceType == Type.BUTTON) || (ai.sourceType == Type.MENU))
            && (ai.source == keyCode)) {
    ...
    if ((keyCode == KeyEvent.KEYCODE_VOLUME_UP)
        || // If these were mapped it would have already returned
        (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN)) return false;
    else {
      int uc = 0;
      if (event != null) uc = event.getUnicodeChar();
  ...
  private float analogCalibrate(float v) {
    if ((v < deadRegion) && (v > -deadRegion)) return 0;
    else {
      if (v > 0) return (v - deadRegion) / (1 - deadRegion);
      else return (v + deadRegion) / (1 - deadRegion);
      // return v;
    }
  }
        ...
        if ((ai.sourceType == Type.ANALOG) && (ai.source != -1)) {
          ...
          else // Must be using analog as a button
          {
            ...
            if (((ai.sourcePositive) && (event.getAxisValue(ai.source)) > 0.5)
              ...
              if (!analogButtonState.get(
                  ai.actionCode)) // Check internal state, only send if different
              {
