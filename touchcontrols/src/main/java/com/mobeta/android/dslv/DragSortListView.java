...
public class DragSortListView extends ListView {
   ...
   * actually means is that the floating View can be dragged above its starting position. Remove in
   * favor of lower-bounding item position?
   */
   ...
   * its height. That process can be expensive, but often the same offscreen item will be requested
   * many times in a row. Once an offscreen item height is calculated, we cache it in this guy.
   * Actually, we cache the height of the child of the DragSortItemView since the item height
   * changes often during a drag-sort.
   */
  ...
  /**
   * The second expanded ListView position that helps represent the drop slot tracking the floating
   * View. This can equal mFirstExpPos if there is no slide shuffle occurring; otherwise it is equal
   * to mFirstExpPos + 1.
   */
  ...
  /**
   * Determines the start of the upward drag-scroll region at the top of the ListView. Specified by
   * a fraction of the ListView height, thus screen resolution agnostic.
   */
  ...
  /**
   * Determines when a slide shuffle animation starts. That is, defines how close to the edge of the
   * drop slot the floating View must be to initiate the slide.
   */
   ...
   * drag-sort animations are turned on). Nearly 1 means the item is at the top of the slide region
   ...
   * inside another View (currently a RelativeLayout) which expands and collapses to simulate the
   * item shuffling.
   */
  ...
  /**
   * Usually called from a FloatViewManager. The float alpha will be reset to the xml-defined value
   * every time a drag is stopped.
   */
   ...
   * @param position
   * @param top
   * @return Shuffle line between position-1 and position (for the given view of the list; that is,
   *     for when top of item at position has y-coord of given `top`). If floating View (treated as
   *     horizontal line) is dropped immediately above this line, it lands in position-1. If dropped
  ...
  /** Cancel a drag. Calls with <code>true</code> as the first argument. */
   ...
   * from the list.
   *
   * @param remove Remove the dragged item from the list. Calls a registered RemoveListener, if one
   ...
   * @param deltaX Offset in x of the touch coordinate from the left edge of the floating View (i.e.
   *     touch-x minus float View left).
   * @param deltaY Offset in y of the touch coordinate from the top edge of the floating View (i.e.
   *     touch-y minus float View top).
   * @return True if the drag was started, false otherwise. This <code>startDrag</code> will fail if
   *     we are not currently in a touch event, there is no registered FloatViewManager, or the
   ...
   *     touch-y minus float View top).
   * @return True if the drag was started, false otherwise. This <code>startDrag</code> will fail if
   *     we are not currently in a touch event, <code>floatView</code> is null, or there is a drag
   *     in progress.
   */
   ...
   * ListAdapter does not return true for {@link ListAdapter#hasStableIds()}, you will need to call
   ...
   * If you have a choiceMode which is not none, this method must be called when the order of items
   * changes in an underlying adapter which does not have stable IDs (see {@link
   ...
   * movable list items: for an adapter that <em>does</em> have stable IDs, ListView will attempt to
   * locate each item based on its ID and move the check state from the item's old position to the
   * new position — which is all fine and good (and removes the need for calling this function),
   * except for the half-baked approach. Apparently to save time in the naive algorithm used,
   * ListView will only search for an ID in the close neighborhood of the old position. If the user
   * moves an item too far (specifically, more than 20 rows away), ListView will give up and just
   * force the item to be unchecked. So if there is a reasonable chance that the user will move
   * items more than 20 rows away from the original position, you may wish to use an adapter with
   * unstable IDs and call this method manually instead.
   *
   * @param from
   * @param to
   */
  ...
  public interface FloatViewManager {
    /**
     * Return the floating View for item at <code>position</code>. DragSortListView will measure and
     * layout this View for you, so feel free to just inflate it. You can help DSLV by setting some
     * {@link ViewGroup.LayoutParams} on this View; otherwise it will set some for you (with a width
     * of FILL_PARENT and a height of WRAP_CONTENT).
     *
     * @param position Position of item to drag (NOTE: <code>position</code> excludes header Views;
     *     thus, if you want to call {@link ListView#getChildAt(int)}, you will need to add {@link
     *     ListView#getHeaderViewsCount()} to the index).
     ...
     * @param location The location (top-left; relative to DSLV top-left) at which the float View
     *     would like to appear, given the current touch location and the offset provided in {@link
     *     DragSortListView#startDrag}.
    ...
    /**
     * Called when the float View is dropped; lets you perform any necessary cleanup. The internal
     * DSLV floating View reference is set to null immediately after this is called.
     *
